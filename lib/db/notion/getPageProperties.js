...
export default async function getPageProperties(
  ...
  if (properties.wordCount !== undefined && properties.wordCount !== null && properties.wordCount !== '') {
    const n = Number(properties.wordCount)
    if (!Number.isNaN(n)) {
      properties.wordCount = n
    }
  }
  ...
  properties.pageCoverPosition = value?.format?.page_cover_position ?? null
