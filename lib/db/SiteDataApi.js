...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  // 站点基础信息
  const siteInfo = getSiteInfo({ collection, block, NOTION_CONFIG, rawMetadata })
  ...
  const allPages = collectionData.filter(post => {
    if (post?.type === 'Post' && post.status === 'Published') {
      ...
      const w = Number.isFinite(post?.wordCount) ? post.wordCount : Number(post?.wordCount)
 ...
 */
function getSiteInfo({ collection, block, NOTION_CONFIG, rawMetadata }) {
  ...
  const pageCover = rawMetadata?.format?.page_cover
    ? mapImgUrl(rawMetadata.format.page_cover, rawMetadata, 'block')
    : rawMetadata?.format?.cover
      ? mapImgUrl(rawMetadata.format.cover, rawMetadata, 'block')
      : collection?.cover
        ? mapImgUrl(collection.cover, collection, 'collection')
        : (() => {
            const databasePageBlockById = pageId ? normalizePageBlock(block?.[pageId]) : null
            const databasePageBlockByParentId = collection?.parent_id ? normalizePageBlock(block?.[collection.parent_id]) : null
            const databasePageBlock = databasePageBlockById || databasePageBlockByParentId
            const databasePageCover = databasePageBlock?.format?.page_cover || databasePageBlock?.format?.cover
            return databasePageCover
              ? mapImgUrl(databasePageCover, databasePageBlock, 'block')
              : defaultPageCover
          })()
  ...
  const pageCoverPosition = rawMetadata?.format?.page_cover_position ?? null
  return { title, description, pageCover, pageCoverPosition, icon, link }
}
